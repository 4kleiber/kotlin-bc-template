...
Infrastructure adapter. Implements the repository ports defined in `domain`.

```
storage/src/main/
├── kotlin/com/akl/bctemplate/storage/   # Exposed ORM repository implementations, one package per bounded context
└── resources/db/migration/               # Flyway SQL migrations (PostgreSQL)
```
...
1. **domain**: write a failing unit test for the new entity's behavior, then the entity (`data class` + behavior methods) to pass it. Define the repository port as a plain interface. Write a failing unit test for the `@DomainService` (using a hand-written in-memory fake repository, not a mocking framework), then the service.
2. **storage**: define the Exposed `Table` object and the adapter implementing the repository port. Add a Flyway migration under `storage/src/main/resources/db/migration/V<next>__<description>.sql`. No config wiring needed — `@Repository` is picked up by Spring Boot's own component scan (`scanBasePackages` already covers `storage`), and the `@DomainService` is picked up by `DomainConfiguration`'s `@ComponentScan` (see Hexagonal Architecture Rules, rule 5).
3. **application**: add a controller (REST, a kotlinx.html page, or both) under a new top-level package named after the bounded context. Write a failing integration test extending `AbstractIntegrationTest` first, then the controller to pass it.
