...
2. **`domain` defines ports as plain interfaces; `storage` implements them as adapters.** Every bounded context in this template is event-sourced (see "Event Sourcing" below), so that's typically *two* ports: an event store (e.g. `NoteEventStore`, the source of truth) and a read-only projection repository (e.g. `NoteRepository`). Their Exposed-backed implementations (`ExposedNoteEventStore`, `ExposedNoteRepository`) live in `storage`. `domain` never imports anything from `storage`.
   Every bounded context's events extend the shared `DomainEvent` abstract class (`domain/DomainEvent.kt`) via its own sealed class (e.g. `NoteEvent`) — that shared shape is what lets every bounded context append to the one system-wide `events` table in `storage` (see "Event Sourcing") instead of each getting its own.
...
├── DomainEvent.kt                # abstract class every bounded context's events extend — see "Event Sourcing"
...
│   ├── eventstore/EventsTable.kt      # the ONE shared events table every bounded context appends to
│   └── notes/                          # this bounded context's event-store adapter + its own projection table/adapter
└── resources/db/migration/               # Flyway SQL migrations (PostgreSQL) — the shared events table (once) + a projection table per bounded context
```
...
- **A shared abstract class, so every bounded context's events fit in one table.** `DomainEvent` (`domain/DomainEvent.kt`) declares the shape every event needs — `aggregateType`, `aggregateId`, `occurredAt` — and each bounded context extends it with its own sealed class (`NoteEvent`) to keep exhaustive `when` matching over its own closed set of event types. `NoteCreated`/`NotePublished`/`NoteArchived` redeclare `aggregateId`/`occurredAt` as `override val` in their own primary constructor specifically so Kotlin's data class equals()/hashCode()/copy() — which only look at a class's own primary-constructor properties, not inherited ones — actually include them.
- **One events table, shared by every bounded context, in the exact order events were created.** `NoteEventStore` (`append`/`loadEvents`) is the source of truth, backed by `storage.eventstore.EventsTable` (a single `events` table, not one per bounded context): a global `id` (BIGSERIAL — the append order across *every* aggregate in *every* bounded context, which is exactly what "the exact order of event creation" means once everything shares one sequence instead of each context getting its own independent one), `aggregate_type` (which bounded context's event hierarchy a row belongs to — `ExposedNoteEventStore` always writes/reads `"Note"`), `aggregate_id`, `version` (the per-aggregate sequence, unique together with `aggregate_type`/`aggregate_id` — see concurrency below), `event_type`, `payload`, `occurred_at`. `NoteRepository` (`findById`/`findAll`, no `save()`) is a read-only query port over `notes_projection` — a per-bounded-context denormalized table shaped like the read model, plus a `version` column recording the last event folded into it. Adding a second bounded context means a second projection table and a second `Exposed*EventStore` tagging its own `aggregate_type` — never a second events table.
...
- **The projection is provably disposable.** `NoteProjectionRebuilder.rebuildAll()` truncates `notes_projection` and replays every `aggregate_type = 'Note'` row from `events` back into it, aggregate by aggregate, using the exact same `Note.apply()` fold. If that produces the same rows the live projection already had (see `NoteEventStoreIntegrationTest`), the projection was never carrying information the event log didn't already have. Filtering by `aggregate_type` here isn't optional once a second bounded context shares the table — without it, `NoteEventCodec.decode()` would choke on a row that isn't a Note event at all.
- **Optimistic concurrency.** `append(aggregateId, expectedVersion, events)` takes the version the caller last saw (0 for a brand-new aggregate); a mismatch throws `ConcurrentEventAppendException` — two commands raced on the same Note. The `UNIQUE (aggregate_type, aggregate_id, version)` index on `events` is the real safety net against a race between the check and the insert; the exception is a friendlier fast-path for the common case.
...
1. **domain**: define the events — a sealed `XEvent` class extending `DomainEvent` (with its own `const val AGGREGATE_TYPE`), plus one data class per fact (past tense, immutable, redeclaring `aggregateId`/`occurredAt` as `override val` in its own primary constructor — see `NoteEvent.kt`'s doc comment for why that's required for correct equality) — then the aggregate (a fold: `X.apply(x, event)` + `X.replay(events)`, plus command methods that return the next event or `null`) — write failing unit tests for both first. Define `XEventStore` (`append`/`loadEvents`) and `XRepository` (`findById`/`findAll`, read-only) as plain interfaces. Write a failing unit test for the `@DomainService` using one fake class that implements *both* ports and applies the same fold internally (see `NoteServiceTest`'s `FakeNoteEventStore` — this is what keeps the fake honest about how the real adapter behaves), then the service.
2. **storage**: no new events table — every bounded context appends to the one existing `storage.eventstore.EventsTable`, tagged with its own `aggregate_type`. Define only a projection table (`x_projection`: the read model's columns plus a `version` bigint) in a Flyway migration under `storage/src/main/resources/db/migration/V<next>__<description>.sql`. Implement `ExposedXEventStore : XEventStore` (insert into `EventsTable` with `aggregateType = XEvent.AGGREGATE_TYPE`, filtering every query the same way; fold each appended event via `X.apply()` to upsert the projection row, in the same transaction) and `ExposedXRepository : XRepository` (read-only queries against the projection table) — see `ExposedNoteEventStore`/`ExposedNoteRepository`/`NoteEventCodec`/`NoteProjection.kt` for the full worked example, including a `ProjectionRebuilder` (optional but recommended, and must also filter by `aggregate_type` when replaying). No config wiring needed — `@Repository`/`@Component` are picked up by Spring Boot's own component scan (`scanBasePackages` already covers `storage`), and the `@DomainService` is picked up by `DomainConfiguration`'s `@ComponentScan` (see Hexagonal Architecture Rules, rule 5).
