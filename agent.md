...
2. **`domain` defines ports as plain interfaces; `storage` implements them as adapters.** Every bounded context in this template is event-sourced (see "Event Sourcing" below) against one shared, generic `EventStore` port — there is no per-bounded-context port (no "NoteEventStore"). `storage`'s `ExposedEventStore` is the only implementation, used by every `@DomainService`. `domain` never imports anything from `storage`.
   A bounded context's own events (e.g. `NoteEvent`) extend the shared `DomainEvent` class directly (see "Event Sourcing") rather than being translated into it — `EventStore.append()` takes `DomainEvent` objects, and `storage` handles only `DomainEvent`/`StoredEvent`, never importing a bounded-context-specific event class.
...
├── DomainEvent.kt                # abstract base class every bounded context's own events extend, + StoredEvent (the read-back materialization)
...
- **One `EventStore`, shared by every bounded context — no "NoteEventStore".** `EventStore` (`append`/`loadEvents`/`listStreamIds`) is the single port every `@DomainService` calls directly; `ExposedEventStore` is its one implementation. Neither knows what a "Note" is — `streamType` is just a string a bounded context's own event hierarchy supplies (`NoteEvent.STREAM_TYPE = "Note"`). Adding a second bounded context adds zero new storage classes: it reuses the same `EventStore` bean with its own `streamType`.
- **`DomainEvent` is an abstract base class a bounded context's own events extend directly — not a separate envelope type they get translated into.** It declares `streamId`, `streamType`, `eventType`, `occurredAt`, `metadata`, and an abstract `fun data(): Map<String, Any?>` for whatever extra business fields a concrete event carries. `NoteEvent`/`NoteCreated`/`NotePublished`/`NoteArchived` extend it directly (see `notes/NoteEvent.kt`) — a `NoteCreated` instance already *is* a `DomainEvent`, handed straight to `EventStore.append(events: List<DomainEvent>)` with no encode step; `List<NoteEvent>` passes there as `List<DomainEvent>` via Kotlin's built-in list covariance. Reading back still needs a decode: `StoredEvent` (also in `DomainEvent.kt`) is the generic materialization of an already-persisted row — everything `DomainEvent` has, plus `id`, `tenantId`, `version`, `sequenceNumber`, and `eventData: Map<String, Any?>` (the row's `data()`) — and a bounded context's own decoder (`NoteEventDecoder.decode(stored: StoredEvent): NoteEvent`) pattern-matches on `eventType` to reconstruct its typed event for replay. That decode step can't be eliminated by any amount of inheritance: turning a generic row back into a specific Kotlin type always needs code that knows that type, and it lives in the bounded context (`domain.notes`), never in `storage`. Only `storage.eventstore.EventsTable` turns a `data()`/`eventData` map into actual JSONB text.
...
1. **domain**: define the events — a sealed `XEvent` class extending `DomainEvent` directly (`const val STREAM_TYPE` in its companion), plus one data class per fact (past tense, immutable, overriding whatever `DomainEvent` properties it needs and implementing `data(): Map<String, Any?>` for its own extra fields) — see `NoteEvent.kt`. Then the aggregate (a fold: `X.apply(x, event)` + `X.replay(events)`, plus command methods that return the next event or `null`) — write failing unit tests for both first. Define `XEventDecoder` (`decode(stored: StoredEvent): XEvent`) — the one direction that can't be eliminated by inheritance, since reconstructing a specific Kotlin type from a generic stored row always needs code that knows that type (see `NoteEventDecoder`). Write the `@DomainService` directly against the shared `EventStore` port (constructor: `class XService(private val eventStore: EventStore)`) — no new port to define; its typed `List<XEvent>` passes straight to `EventStore.append(events: List<DomainEvent>)` via Kotlin's list covariance, no encode step needed. Write a failing unit test for it first using `domain`'s shared `FakeEventStore` (`domain/src/test/kotlin/com/akl/bctemplate/domain/FakeEventStore.kt`) — a single hand-written in-memory fake reused by every bounded context's `@DomainService` tests, since `EventStore` itself is generic; no need to write another one, then the service.
